import React from 'react'
import axios from 'axios'


export default class Auto1 extends React.Component{
    constructor(props){
        super(props)
        this.state={
            suggestion:[],
            text:"",
            Circle:[],
            Circleautoid:''
        }
    }
    fetch(){
        axios.get(`/getCircleName?circleName=${this.state.text}`)
        .then(x => {
            console.log(x);
            this.setState({Circle: x.data});
            //console.log(this.state.Circle[0][1])
            this.setState({Circleautoid:this.state.Circle[0][1]})
            console.log(this.state.Circleautoid)
          })
       
    }
    
    onTextchange=(e)=>{
        const {items}=this.props
        const value=e.target.value
        let suggestion=[]
        if(value.length){
            const regex = new RegExp(`^${value}`,'i')
            suggestion=items.sort().filter(v=>regex.test(v))
        }
        this.setState(()=>({ suggestion,text:value }))
    }
    suggestionselected(value){
        this.fetch()
        this.setState(()=>({
            text:value,
            suggestion:[]
        }))
        
    }

    rendersuggestion(){
        const {suggestion}=this.state
        if(suggestion.length===0){
            return null
        }
        return(
                <div>
                    {suggestion.map((items)=><ol  onClick={()=>this.suggestionselected(items)}>{items}</ol>)}
                </div>

        )
    }


    

    render(){
        const {text}=this.state
        var store=this.state.Circleautoid;
        localStorage.setItem ('data',store);
        return(     <div>
                <table>
                <label className="label">{this.props.label}</label>
                        <div className="AutocompleteText input">
                            <input  value={text} onChange={this.onTextchange}  />
                            {this.rendersuggestion()}
                            
                        </div>
                </table>
                </div>
        )
    }
}